...
public class Logger {
   ...
   * 监听业务层全部查询的返回值并输出。
   */
  @AfterReturning(value = "execution(* net.penyo.herbms.service.*.select*(..))", returning = "result")
   ...
   * 监听业务层全部查询的异常并输出。
   */
  @AfterThrowing(value = "execution(* net.penyo.herbms.service.*.*(..))", throwing = "exception")
   ...
   * 监听业务层全部查询的必要信息并输出。
   */
  @Around(value = "execution(* net.penyo.herbms.service.*.*(..))")
   ...
   * 监听业务层全部查询的参数并输出。
   */
  @Before(value = "execution(* net.penyo.herbms.service.*.*(..))")
   ...
   * 监听业务层全部查询的耗时并输出。
   */
  @Around(value = "execution(* net.penyo.herbms.service.*.select*(..))")
