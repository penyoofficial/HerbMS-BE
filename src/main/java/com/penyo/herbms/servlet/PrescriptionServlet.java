...
public class PrescriptionServlet extends GenericServlet<PrescriptionInfoBean, PrescriptionBean, PrescriptionInfoService, PrescriptionService> {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) {
    ...
    if (!req.getServletPath().contains("Specific"))
      doProcess(resp, new PrescriptionInfoService(), new PrescriptionService(), true);
    else doSpecificProcess(resp, new PrescriptionInfoService(), new PrescriptionService());
    doEraseParams();
  }

  @Override
  @SuppressWarnings("unchecked")
  public void doSpecificProcess(HttpServletResponse resp, PrescriptionInfoService piService, PrescriptionService pService) {
    if (needQueryA) {
      List<String> idtis = new ArrayList<>();

      ReturnDataPack<PrescriptionInfoBean> pis = (ReturnDataPack<PrescriptionInfoBean>) doProcess(resp, piService, pService, false);
      ...
      ReturnDataPack<PrescriptionBean> ps = (ReturnDataPack<PrescriptionBean>) doProcess(resp, piService, pService, false);
