...
public class PrescriptionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
            ...
            try {
                ...
                if (opType == null) {
                    if (needQueryInfo) {
                        ...
                        if (piService.selectById(id) == null)
                            piService.add(pi);
                        else
                            piService.update(pi);
                    } else {
                        PrescriptionBean p = new PrescriptionBean(
                                ...
                                params.get("dosage"),
                                params.get("usage"));
                        if (pService.selectById(id) == null)
                            pService.add(p);
                        else
                            pService.update(p);
                    }
        ...
        if (isId) {
            if (needQueryInfo)
                pis.add(piService.selectById(Integer.parseInt(keyword)));
            else
                ps.add(pService.selectById(Integer.parseInt(keyword)));
        } else {
            if (needQueryInfo)
                pis = piService.selectByField(keyword);
            else
                ps = pService.selectByField(keyword);
        }
